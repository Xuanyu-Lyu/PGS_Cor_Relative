...
5) AM-Migration model, no vertical transmission (simplified 04AMmigration):
...
Usage:
    python DataGeneratingNN_Combined_05AMmigration_noVT.py
    (Run via SLURM array job - see submit_datagenerating_nn_combined_05AMmigration_noVT.sh)
"""
...
# Directory setup
PROJECT_BASE = Path("/projects/xuly4739/Py_Projects/PGS_Cor_Relative/Data/DataGeneratingNN_Paper/05AMmigration_noVT")
...
FIXED_PARAMS = {
    ...
    'f22': 0.0,               # No vertical transmission for migration
...
def setup_matrices(params):
    """
    Setup covariance and other matrices for the 05AMmigration_noVT condition.
...
def run_condition(condition, project_base):
    ...
    print(f"# Parameters (05AMmigration_noVT):")
    ...
    print(f"#   f11={condition['f11']:.4f}, f22={condition['f22']:.4f} (both fixed=0) "
          f"(no vertical transmission; f12=f21=0, s=0)")
...
def main():
    ...
    print("Condition: 05AMmigration_noVT (EA mating + migration trait, no VT)")
    print("="*70)
