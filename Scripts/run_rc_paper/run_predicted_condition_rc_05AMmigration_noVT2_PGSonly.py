...
Condition 05AMmigration_noVT2_PGSonly:
  ...
  - Only am11 and move_p are free parameters, set to the posterior means from
    the NPE fit trained on PGS-only observed correlations
    (results_npe_unweighted_05AMmigration_noVT2_PGSonly); all other parameters
    are fixed at the values used to generate the NN training data
    (see DataGeneratingNN_Combined_05AMmigration_noVT.py).
"""
...
SCRATCH_BASE = Path("/scratch/alpine/xuly4739/PGS_Cor_Relative/Data/predicted_condition_05AMmigration_noVT2_PGSonly")
PROJECT_BASE = Path("/projects/xuly4739/Py_Projects/PGS_Cor_Relative/Data/predicted_condition_05AMmigration_noVT2_PGSonly")

# Condition 05AMmigration_noVT2_PGSonly: AE model with latent AM on trait 1 and
# island migration driven by trait 2.  Only am11 and move_p were free in the
# PGS-only NPE fit (posterior means below); everything else is fixed at the
# training-data values from DataGeneratingNN_Combined_05AMmigration_noVT.py.
CONDITION = {
    'name': 'Predicted_Condition_05AMmigration_noVT2_PGSonly',
    # Free parameters (NPE posterior means, PGS-only fit)
    'am11':   0.3032,  # within-island spousal correlation on EA (trait 1)
    'move_p': 0.1462,  # fraction migrating per generation
    # Trait 1 (EA): fixed
    'prop_h2_latent1': 0.6,
    'vg1': 0.6,
    'f11': 0.0,       # no vertical transmission
    # Trait 2 (migration): fully latent, no AM, no VT (fixed)
    'prop_h2_latent2': 1.0,
    'vg2': 0.8,
    'am22': 0.0,
    'f22': 0.0,
    # Cross-trait parameters (fixed)
    'f12': 0.0,
    'f21': 0.0,
    'am12': 0.0,
    'am21': 0.0,
    'rg': 0.2,        # genetic correlation
    're': 0.2,        # environmental correlation
}
...
def setup_matrices(params):
    """
    Build covariance matrices for condition 05AMmigration_noVT2_PGSonly.

    Trait 1: AE model, latent AM (am11), no VT (f11=0).
    Trait 2: fully latent (prop_h2_latent2=1.0), no direct AM, no VT.
    Cross-trait: rg, re.
    ...
    # Vertical transmission matrix (all f's fixed at 0 for this condition)
...
def run_predicted_condition(condition, scratch_base, project_base):
    ...
    print(f"# Condition 05AMmigration_noVT2_PGSonly: AE model, latent AM on trait 1, island migration on trait 2, no VT")
    print(f"# Free (posterior means, PGS-only NPE fit): am11={condition['am11']:.4f}, move_p={condition['move_p']:.4f}")
    print(f"# Trait 1 (AE+latentAM, fixed): prop_h2_latent1={condition['prop_h2_latent1']:.4f}, "
          f"vg1={condition['vg1']:.4f}, f11={condition['f11']:.4f}")
    print(f"# Trait 2 (migration, fully latent, fixed): prop_h2_latent2={condition['prop_h2_latent2']:.4f}, "
          f"vg2={condition['vg2']:.4f}, f22={condition['f22']:.4f}")
    print(f"# Islands: {N_ISLANDS}")
    print(f"# Cross-trait (fixed): rg={condition['rg']:.4f}, re={condition['re']:.4f}")
...
def main():
    print("\n" + "="*70)
    print("PREDICTED CONDITION SIMULATION SCRIPT - 05AMmigration_noVT2_PGSonly")
    print("="*70)
    ...
    print(f"\nFree parameters (NPE posterior means, PGS-only fit):")
    print(f"  am11={CONDITION['am11']:.4f},  move_p={CONDITION['move_p']:.4f}")
    print(f"\nTrait 1 parameters (AE + latent AM, fixed):")
    print(f"  prop_h2_latent1={CONDITION['prop_h2_latent1']:.4f},  vg1={CONDITION['vg1']:.4f},  "
          f"f11={CONDITION['f11']:.4f}")
    print(f"\nTrait 2 parameters (fully latent migration trait, fixed):")
