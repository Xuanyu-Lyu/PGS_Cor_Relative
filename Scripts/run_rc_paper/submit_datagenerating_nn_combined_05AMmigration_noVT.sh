...
#SBATCH --job-name=nn_05migration_noVT
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=slurm_logs/nn_05migration_noVT_%A_%a.out
#SBATCH --error=slurm_logs/nn_05migration_noVT_%A_%a.err
...
echo "Condition: 05AMmigration_noVT"
...
echo "  No vertical transmission (f11=f22=0); no shared env (s=0); 40 generations"
...
python -u DataGeneratingNN_Combined_05AMmigration_noVT.py
...
if [ "$SLURM_ARRAY_TASK_ID" == "500" ]; then
    ...
    echo "================================================"
    python -u CombineNN_Data_Large_05AMmigration_noVT.py --split --test_size 0.2
