...
# Bivariate AE + vertical transmission + latent AM condition (03_EnvLatentAM).
# Single-trait phenotypic AM on trait 2 only (latent factor, am22);
# traits are genetically correlated (rg). Within-trait vertical transmission
# (f11, f22) and shared family environment (s = 0.5) are present.
# Free parameters (vg1, f11, f22, am22, rg) are posterior means from the NPE fit to
# the observed data (results_npe_unweighted_03EnvLatentAM); all other parameters are
# fixed at the values used to generate the NN training data (see FIXED_PARAMS in
# DataGeneratingNN_Combined_03EnvLatentAM.py).
CONDITION = {
    'name': 'Predicted_Condition_03EnvLatentAM',
    # Trait 1 (posterior means for vg1, f11; prop_h2_latent1 fixed)
    'prop_h2_latent1': 0.7,   # fixed
    'vg1': 0.4523,
    'am11': 0.0,      # no direct AM on trait 1
    'f11': 0.0575,
    # Trait 2: latent AM (posterior means for am22, f22; prop_h2_latent2, vg2 fixed)
    'prop_h2_latent2': 1.0,   # fixed
    'vg2': 0.5,       # fixed
    'am22': 0.6225,   # latent AM on trait 2 (single-trait mating)
    'f22': 0.1151,
    's11': 0.5,       # fixed shared family environment
    's12': 0.5,       # fixed cross-trait shared environment
    's21': 0.5,       # fixed cross-trait shared environment
    's22': 0.5,       # fixed shared family environment
    # Cross-trait parameters
    'f12': 0.0,       # fixed: no cross-trait vertical transmission
    'f21': 0.0,       # fixed: no cross-trait vertical transmission
    'am12': 0.0,
    'am21': 0.0,
    'rg': 0.8512,     # genetic correlation (posterior mean)
    're': 0.1,        # fixed environmental correlation
}
...
def setup_matrices(params):
    ...
    Condition 03_EnvLatentAM: bivariate AE model with vertical transmission and single-trait latent AM on trait 2.
    Trait 1: AE + VT (f11), no direct AM (am11=0)
    Trait 2: fully latent, VT (f22), latent AM via am22 (MATE_ON_TRAIT controls which trait is mated on)
    Cross-trait: genetic correlation rg, fixed environmental correlation re, fixed shared environment s
    """
    vg1 = params['vg1']
    vg2 = params['vg2']
    rg = params['rg']
    ...
    # Assortative mating: single-trait or bivariate depending on MATE_ON_TRAIT
    if MATE_ON_TRAIT == 1:
    ...
    return {
        ...
        'd_mat': delta_mat,
        'am_list': am_list,
        'mate_on_trait': MATE_ON_TRAIT if MATE_ON_TRAIT in (1, 2) else None,
        'covy_mat': covy_mat,
        'k2_matrix': k2_matrix
    }
...
def run_single_iteration(iteration, condition_name, params, matrices, scratch_dir):
    ...
    # Extract mate_on_trait before unpacking matrices into sim kwargs
...
def run_predicted_condition(condition, scratch_base, project_base):
    ...
    print(f"# Condition 03_EnvLatentAM: bivariate AE + VT model, single-trait latent AM on trait {MATE_ON_TRAIT}")
    print(f"# Trait 1 (AE+VT): vg1={condition['vg1']:.4f}, f11={condition['f11']:.4f}, prop_h2_latent1={condition['prop_h2_latent1']:.4f}")
    print(f"# Trait 2 (latent AM+VT): am22={condition['am22']:.4f}, f22={condition['f22']:.4f}, vg2={condition['vg2']:.4f}")
...
def main():
    ...
    print(f"\nTrait 1 parameters (AE + VT, 03EnvLatentAM posterior means; prop_h2_latent1 fixed):")
    print(f"  prop_h2_latent1={CONDITION['prop_h2_latent1']:.4f}, vg1={CONDITION['vg1']:.4f}, am11={CONDITION['am11']:.4f}")
    print(f"\nTrait 2 parameters (latent AM + VT, 03EnvLatentAM posterior means; prop_h2_latent2, vg2 fixed):")
    print(f"  prop_h2_latent2={CONDITION['prop_h2_latent2']:.4f}, vg2={CONDITION['vg2']:.4f}, am22={CONDITION['am22']:.4f}")
    print(f"\nCross-trait (rg posterior mean, re fixed): rg={CONDITION['rg']:.4f}, re={CONDITION['re']:.4f}")
    print("="*70 + "\n")
    ...
    print("\n" + "="*70)
    print(f"SIMULATION COMPLETED - {CONDITION['name']} (03_EnvLatentAM posterior means, MATE_ON_TRAIT={MATE_ON_TRAIT})")
    print("="*70)
