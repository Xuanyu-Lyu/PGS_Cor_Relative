...
Condition 05AMmigration_noVT:
  ...
  - Only am11 and move_p are free parameters, set to the posterior means from
    the NPE fit to the observed data; all other parameters are fixed at the
    values used to generate the NN training data
...
SCRATCH_BASE = Path("/scratch/alpine/xuly4739/PGS_Cor_Relative/Data/predicted_condition_05AMmigration_noVT")
PROJECT_BASE = Path("/projects/xuly4739/Py_Projects/PGS_Cor_Relative/Data/predicted_condition_05AMmigration_noVT")

# Condition 05AMmigration_noVT: AE model with latent AM on trait 1 and island
# migration driven by trait 2.  Only am11 and move_p were free in the NPE fit
# (posterior means below); everything else is fixed at the training-data values
# from DataGeneratingNN_Combined_05AMmigration_noVT.py.
CONDITION = {
    'name': 'Predicted_Condition_05AMmigration_noVT',
    # Free parameters (NPE posterior means)
    'am11':   0.3404,  # within-island spousal correlation on EA (trait 1)
    'move_p': 0.2142,  # fraction migrating per generation
    # Trait 1 (EA): fixed
    'prop_h2_latent1': 0.6,
    'vg1': 0.6,
...
def setup_matrices(params):
    """
    Build covariance matrices for condition 05AMmigration_noVT.
...
def run_predicted_condition(condition, scratch_base, project_base):
    ...
    print(f"# Condition 05AMmigration_noVT: AE model, latent AM on trait 1, island migration on trait 2, no VT")
    print(f"# Free (posterior means): am11={condition['am11']:.4f}, move_p={condition['move_p']:.4f}")
...
def main():
    print("\n" + "="*70)
    print("PREDICTED CONDITION SIMULATION SCRIPT - 05AMmigration_noVT")
    print("="*70)
