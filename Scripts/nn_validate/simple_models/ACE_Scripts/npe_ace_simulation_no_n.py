"""
NPE-based ACE Simulation Study — model trained WITHOUT N_pairs as a feature
...
(50, 100, 200, 500, 1000, 2000):
  ...
  3. Feed ONLY those 4 statistics into the trained NPE posterior (N is NOT passed).
  4. Draw posterior samples and compute posterior mean (point estimate) and
     posterior SD (SE analogue).

This script is the counterpart to npe_ace_simulation.py and is intended for
evaluating a model that was trained without N_pairs / log_N_pairs as a feature,
so that the two models can be compared on the same test conditions.

Output:
  npe_simulation_results_no_n.csv  — one row per condition × sample size.
...
from train_ace_nn import ACEEmbeddingNet, ACE_PARAM_NAMES, map_from_samples  # noqa: F401
...
def load_posterior(model_dir: Path):
    ...
    Raises a warning if the config indicates N_pairs / log_N_pairs was
    included as a feature, since this script is designed for N-free models.
    """
    ...
    if ("N_pairs" in feature_cols) or ("log_N_pairs" in feature_cols):
        warnings.warn(
            f"Model in '{model_dir}' was trained WITH N_pairs as a feature "
            f"(feature_cols={feature_cols}). This script is designed for models "
            "trained WITHOUT N_pairs. Use npe_ace_simulation.py instead.",
            UserWarning,
            stacklevel=2,
        )

    # Retain only the 4 covariance features in their original order
    cov_features = [f for f in feature_cols
                    if f not in ("N_pairs", "log_N_pairs")]
    ...
    return posterior, scaler, cov_features, param_names
...
def run_simulation(conditions_csv: str,
    ...
    print("\n" + "=" * 70)
    print("NPE ACE SIMULATION STUDY  (no N_pairs feature)")
    print("=" * 70)
    ...
    # ---- Load model --------------------------------------------------------
    posterior, scaler, cov_features, param_names = load_posterior(model_dir)
    print(f"Loaded posterior  — features   : {cov_features}")
    print(f"                  — params     : {param_names}")
    print(f"                  — N feature  : not used\n")
    ...
    for _, cond in cond_df.iterrows():
        ...
        for N in sample_sizes:
            ...
            # ---- Build feature vector (4 covariance features only) ---------
            x_raw    = np.array([mz_var, mz_cov, dz_var, dz_cov],
...
def main():
    parser = argparse.ArgumentParser(
        description="NPE ACE simulation study — model trained without N_pairs feature"
    )
    parser.add_argument(
        "--conditions",
        type=str,
    ...
    parser.add_argument(
        "--model_dir",
        type=str,
        default="results_ace_npe_no_n",
        help="Directory containing trained NPE posterior without N feature "
             "(default: results_ace_npe_no_n)",
    )
    parser.add_argument(
        "--sample_sizes",
        ...
        help="List of twin-pair sample sizes to evaluate (default: 50 100 200 500 1000 2000)",
    )
    parser.add_argument(
        "--n_posterior",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default=str(SCRIPT_DIR / "npe_simulation_results_no_n.csv"),
        help="Output CSV path (default: npe_simulation_results_no_n.csv)",
    )
