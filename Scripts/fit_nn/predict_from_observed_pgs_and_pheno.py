...
correlation data. Observed data are loaded from two separate CSV files:
...
def prepare_correlation_input(pgs_correlations_dict, pheno_correlations_dict, expected_features):
    ...
    # Track which correlations from input were used
    used_pgs_rels = set()
    used_pheno_rels = set()
    ...
    for i, feature_name in enumerate(expected_features):
        ...
        if '_PGS1' in feature_name:
            # PGS feature - remove _PGS1 suffix
            ...
            # Phenotypic feature - remove _Y1 suffix
    ...
    # Check for unused correlations (provided but not in model)
...
def main():
    parser = argparse.ArgumentParser(description='Predict parameters from observed PGS + phenotypic correlations')
    parser.add_argument('--model_dir', type=str, default='results_pgs_and_pheno',
    ...
    print("\n" + "="*70)
    print("PARAMETER PREDICTION FROM OBSERVED PGS + PHENOTYPIC CORRELATIONS")
    print("="*70)
    ...
    if args.correlations_pgs:
        # Load PGS correlations from file
        ...
        # Get PGS correlations from command-line arguments
    ...
    if args.correlations_pheno:
        # Load phenotypic correlations from file
        ...
        # Get phenotypic correlations from command-line arguments
        rel_types = ['S', 'HSFS', 'PSC', 'PPSCC', 'M', 'MS', 'SMS', 'MSC', 'MSM', 
                     'SMSC', 'SMSM', 'SMSMS', 'MSMSM', 'MSMSC', 'PSMSC', 'SMSMSC', 'MSMSMS']
    ...
    if not pgs_correlations_dict and not pheno_correlations_dict:
        ...
        print("\nExample:")
        print("  python predict_from_observed_pgs_and_pheno.py \\")
